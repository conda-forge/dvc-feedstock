...
Once the `conda-forge` channel has been enabled, `dvc, dvc-azure, dvc-base, dvc-gdrive, dvc-gs, dvc-hdfs, dvc-oss, dvc-s3, dvc-ssh` can be installed with:

```
conda install dvc dvc-azure dvc-base dvc-gdrive dvc-gs dvc-hdfs dvc-oss dvc-s3 dvc-ssh
```
